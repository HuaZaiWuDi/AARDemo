package com.today.step.lib.util;
...
public class JLoggerWraper {
    ...
    public static final void initXLog(Application application, String xlogPath) {
        String fileName = com.andrjhf.lib.jlogger.Utils.getProcessName(application);
        try {
...
//            JLogger.i(context.getPackageName(), String.format("%s : %s", eventID, label));
            Log.i(context.getPackageName(), String.format("%s : %s", eventID, label));
        } else {
//            JLogger.i(context.getPackageName(), eventID);
            Log.i(context.getPackageName(), eventID);
        }
    }
...
//        JLogger.flush();
    ...
    public synchronized static final void deviceInfo(Context context) {
        ...
        map.put("APP_Version", com.andrjhf.lib.jlogger.Utils.getAppVersion(context));
        map.put("APP_Build", com.andrjhf.lib.jlogger.Utils.getAppVersionCode(context));
        //1. 手机具体型号，设备信息
        //2. 早上打开，晚上打开
        onEventInfo(context, com.andrjhf.lib.jlogger.JLoggerConstant.JLOGGER_DEVICE_INFO, map);
    }
}
