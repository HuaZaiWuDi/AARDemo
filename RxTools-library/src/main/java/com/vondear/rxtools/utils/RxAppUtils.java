...
public class RxAppUtils {
    ...
    public static boolean installAppSilent(Context context, String filePath) {
        File file = RxFileUtils.getFileByPath(filePath);
        if (!RxFileUtils.isFileExists(file)) return false;
        String command = "LD_LIBRARY_PATH=/vendor/lib:/system/lib pm install " + filePath;
        RxShellUtils.CommandResult commandResult = RxShellUtils.execCmd(command, !isSystemApp(context), true);
    ...
    public static boolean isSystemApp(Context context, String packageName) {
        if (RxDataUtils.isNullString(packageName)) return false;
        try {
    ...
    public static void uninstallApp(Context context, String packageName) {
        if (RxDataUtils.isNullString(packageName)) return;
        context.startActivity(RxIntentUtils.getUninstallAppIntent(packageName));
    }
    ...
    public static void uninstallApp(Activity activity, String packageName, int requestCode) {
        if (RxDataUtils.isNullString(packageName)) return;
        activity.startActivityForResult(RxIntentUtils.getUninstallAppIntent(packageName), requestCode);
    }
    ...
    public static boolean isAppRoot() {
        RxShellUtils.CommandResult result = RxShellUtils.execCmd("echo root", true);
        if (result.result == 0) {
            return true;
        }
    ...
    public static void launchApp(Context context, String packageName) {
        if (RxDataUtils.isNullString(packageName)) return;
        context.startActivity(RxIntentUtils.getLaunchAppIntent(context, packageName));
    }
    ...
    public static void launchApp(Activity activity, String packageName, int requestCode) {
        if (RxDataUtils.isNullString(packageName)) return;
        activity.startActivityForResult(RxIntentUtils.getLaunchAppIntent(activity, packageName), requestCode);
    }
    ...
    public static void getAppDetailsSettings(Context context, String packageName) {
        if (RxDataUtils.isNullString(packageName)) return;
        context.startActivity(RxIntentUtils.getAppDetailsSettingsIntent(packageName));
    }
    ...
    public static String getAppName(Context context, String packageName) {
        if (RxDataUtils.isNullString(packageName)) return null;
        try {
            PackageManager pm = context.getPackageManager();
    ...
    public static int getAppVersionCode(Context context, String packageName) {
        if (RxDataUtils.isNullString(packageName)) return -1;
        try {
            PackageManager pm = context.getPackageManager();
            PackageInfo pi = pm.getPackageInfo(packageName, 0);
    ...
    public static String getAppSignatureSHA1(Context context, String packageName) {
        ...
        return RxEncryptUtils.encryptSHA1ToString(signature[0].toByteArray()).
...
//        getAllProgramInfo(systemAppList, context, SYSTEM_APP);
        ...
        for (AppInfo info : allAppsInfo) {
            if (isSystemApp(context, info.getPackageName())) {
    ...
    public static boolean cleanAppData(Context context, File... dirs) {
        ...
        isSuccess &= RxFileUtils.cleanInternalFiles(context);
        isSuccess &= RxFileUtils.cleanExternalCache(context);
        for (File dir : dirs) {
            isSuccess &= RxFileUtils.cleanCustomCache(dir);
        }
        return isSuccess;
    }
}