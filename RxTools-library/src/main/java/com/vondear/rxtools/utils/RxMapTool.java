...
public class RxMapTool {
    ...
    public static void openMap(Context mContext, Gps gpsFrom, Gps gpsTo, String storeName) {
        //检测设备是否安装高德地图APP
        if (RxPackageManagerTool.haveExistPackageName(mContext, GAODE_PACKAGE_NAME)) {
            RxMapTool.openGaodeMapToGuide(mContext, gpsFrom, gpsTo, storeName);
            //检测设备是否安装百度地图APP
        } else if (RxPackageManagerTool.haveExistPackageName(mContext, BAIDU_PACKAGE_NAME)) {
    ...
    public static void openGaodeMapToGuide(Context mContext, Gps gpsFrom, Gps gpsTo, String storeName) {
        ...
        Gps gps = RxLocationUtils.GPS84ToGCJ02(gpsFrom.getWgLon(), gpsFrom.getWgLat());
        Gps gps1 = RxLocationUtils.GPS84ToGCJ02(gpsTo.getWgLon(), gpsTo.getWgLat());
    ...
    public static void openBaiduMapToGuide(Context mContext, Gps gps, String storeName) {
        Intent intent = new Intent();
        Gps gps1 = RxLocationUtils.GPS84ToGCJ02(gps.getWgLon(), gps.getWgLat());
        Gps gps2 = RxLocationUtils.GCJ02ToBD09(gps1.getWgLon(), gps1.getWgLat());
        String url = "baidumap://map/direction?" +
                "destination=name:" + storeName +
                "|latlng:" + gps2.getWgLat() + "," + gps2.getWgLon() +
                "&mode=driving" +
                "&sy=3" +
                "&index=0" +
                "&target=1";
    ...
    public static void openBrowserToGuide(Context mContext, Gps gpsFrom, String storeName) {
        ...
        String url = "http://uri.amap.com/navigation?" +
                "to=" + gps.getWgLat() + "," + gps.getWgLon() + "," + storeName + "" +
                "&mode=car" +
                "&policy=1" +
                "&src=mypage" +
                "&coordinate=gaode" +
