...
public class RxDataUtils {
    ...
    public static double byte2Size(long byteNum, RxConstUtils.MemoryUnit unit) {
        ...
        switch (unit) {
            default:
            case BYTE:
                return (double) byteNum / RxConstUtils.BYTE;
            case KB:
                return (double) byteNum / RxConstUtils.KB;
            case MB:
                return (double) byteNum / RxConstUtils.MB;
            case GB:
                return (double) byteNum / RxConstUtils.GB;
        }
    }
    ...
    public static long size2Byte(long size, RxConstUtils.MemoryUnit unit) {
        ...
        switch (unit) {
            default:
            case BYTE:
                return size * RxConstUtils.BYTE;
            case KB:
                return size * RxConstUtils.KB;
            case MB:
                return size * RxConstUtils.MB;
            case GB:
                return size * RxConstUtils.GB;
        }
    }
    ...
    public static String byte2FitSize(long byteNum) {
        ...
        } else if (byteNum < RxConstUtils.KB) {
            return String.format(Locale.getDefault(), "%.3fB", (double) byteNum);
        } else if (byteNum < RxConstUtils.MB) {
            return String.format(Locale.getDefault(), "%.3fKB", (double) byteNum / RxConstUtils.KB);
        } else if (byteNum < RxConstUtils.GB) {
            return String.format(Locale.getDefault(), "%.3fMB", (double) byteNum / RxConstUtils.MB);
        } else {
            return String.format(Locale.getDefault(), "%.3fGB", (double) byteNum / RxConstUtils.GB);
        }
    }
    ...
    public static ByteArrayOutputStream input2OutputStream(InputStream is) {
        ...
        try {
            ...
            byte[] b = new byte[RxConstUtils.KB];
            int len;
            while ((len = is.read(b, 0, RxConstUtils.KB)) != -1) {
                os.write(b, 0, len);
            }
            return os;
        } catch (IOException e) {
