...
//            Cursor cursor = context.getContentResolver().query(uri, new String[]{MediaStore.Images.ImageColumns.DATA}, null, null, null);
    ...
    public static String getImageAbsolutePath(Context context, Uri imageUri) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.KITKAT && DocumentsContract.isDocumentUri(context, imageUri)) {
            if (RxFileUtils.isExternalStorageDocument(imageUri)) {
            ...
            } else if (RxFileUtils.isDownloadsDocument(imageUri)) {
                String id = DocumentsContract.getDocumentId(imageUri);
                Uri contentUri = ContentUris.withAppendedId(Uri.parse("content://downloads/public_downloads"), Long.valueOf(id));
                return RxFileUtils.getDataColumn(context, contentUri, null, null);
            } else if (RxFileUtils.isMediaDocument(imageUri)) {
                String docId = DocumentsContract.getDocumentId(imageUri);
                String[] split = docId.split(":");
                String type = split[0];
                Uri contentUri = null;
                ...
                return RxFileUtils.getDataColumn(context, contentUri, selection, selectionArgs);
            }
        } // MediaStore (and general)
        else if ("content".equalsIgnoreCase(imageUri.getScheme())) {
            // Return the remote address
            if (RxFileUtils.isGooglePhotosUri(imageUri))
                return imageUri.getLastPathSegment();
            return RxFileUtils.getDataColumn(context, imageUri, null, null);
        }
        // File
