package com.vondear.rxtools.view.nesetedscroll;
...
/**
 * 1. 向上滑动顶部WebView时，首先滑动WebView的内容，WebView的内容滑动到底后再滑动外层容器。
 * 外层容器滑动到RecyclerView完全露出后，再将滑动距离或者剩余速度传递给RecyclerView继续滑动.
 * <p>
 * 2. 滑动底部RecyclerView时，首先滑动RecyclerView的内容，RecyclerView的内容滑动到顶后再滑动外层容器。
 * 外层容器也滑动到顶后，再将滑动距离或者剩余速度传递给WebView继续滑动.
 * <p>
 * 3. 触摸本身不可滑动的TextView时，滑动事件被外层容器拦截。
 * 外层容器根据滑动方向和是否滑动到相应阈值，再将相应的滑动距离或者速度传递给WebView或者RecyclerView.
 * <p>
 * 再不知道NestedScrolling机制之前，我相信大部分人想实现上面的滑动效果都是比较头大的，
 * 特别是滑动距离和速度要从WebView->外层容器->RecyclerView并且还要支持反向传递。
 */
