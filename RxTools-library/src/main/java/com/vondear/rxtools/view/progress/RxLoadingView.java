 ...
 * @date 2016-09-21
 */
public class RxLoadingView extends View {
    ...
    public RxLoadingView(Context context) {
        this(context, null);
    }

    public RxLoadingView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public RxLoadingView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.RxLoadingView, defStyleAttr, 0);
        mSize = array.getDimensionPixelSize(R.styleable.RxLoadingView_loading_view_size, RxUtils.dp2px(32));
        mPaintColor = array.getInt(R.styleable.RxLoadingView_view_color, Color.WHITE);
        array.recycle();
        initPaint();
    }

    public RxLoadingView(Context context, int size, int color) {
        super(context);
        mSize = size;
        mPaintColor = color;
        initPaint();
