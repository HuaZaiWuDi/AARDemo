...
import com.wesmarclothing.qrscanner.scaner.BitmapLuminanceSource;
...
public class RxQrBarTool {
    ...
    public static Result decodeFromPhoto(Bitmap photo) {
        ...
        if (photo != null) {
            Bitmap smallBitmap = zoomImage(photo, photo.getWidth() / 2, photo.getHeight() / 2);// 为防止原始图片过大导致内存溢出，这里先缩小原图显示，然后释放原始Bitmap占用的内存
